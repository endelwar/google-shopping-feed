...
class Item
{
    ...
    /**
     * Set the link/URL of the product
     *
     * @param string $link
     */
    ...
    /**
     * Set the price of the product, do not format before passing
     *
     * @param string $price
    ...
    {
        $node = new Node('price');
        $this->nodes['price'] = $node->value(
            number_format($price, 2, '.', '') . ' ' . $currency
        )->_namespace($this->namespace);
    }

    /**
     * Set the sale price, do not format before passing
     *
     * @param string $salePrice
     * @param string $currency
     */
    public function sale_price($salePrice, $currency)
    {
        $node = new Node('sale_price');
        $this->nodes['sale_price'] = $node->value(
    ...
    public function description($description)
    {
        ...
        $this->nodes['description'] = $node->value(
            substr($description, 0, 5000)
        )->_namespace($this->namespace)->addCdata();
    }

    /**
     * Set the condition of the product (pass in the constants above to standardise the values)
     *
     ...
     * @param string $currency
     */
    public function shipping($code, $service, $cost, $currency)
    {
        $node = new Node('shipping');
        $value = "<g:country>{$code}</g:country><g:service>{$service}</g:service><g:price>" .
